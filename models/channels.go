...
var ChannelWhere = struct {
	...
	ChannelID: whereHelperstring{field: "`channels`.`channel_id`"},
...
var (
	channelAllColumns            = []string{"channel_id", "name", "created_at", "updated_at"}
	channelColumnsWithoutDefault = []string{"channel_id", "name"}
	channelColumnsWithDefault    = []string{"created_at", "updated_at"}
	channelPrimaryKeyColumns     = []string{"channel_id"}
)
...
func (o *Channel) FollowedChannels(mods ...qm.QueryMod) followedChannelQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`followed_channels`.`channel_id`=?", o.ChannelID),
	)
...
func (channelL) LoadFollowedChannels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeChannel interface{}, mods queries.Applicator) error {
	var slice []*Channel
	var object *Channel
	...
	query := NewQuery(qm.From(`followed_channels`), qm.WhereIn(`followed_channels.channel_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Channel) AddFollowedChannels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*FollowedChannel) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ChannelID, rel.FollowedChannelID}
...
func FindChannel(ctx context.Context, exec boil.ContextExecutor, channelID string, selectCols ...string) (*Channel, error) {
	...
	query := fmt.Sprintf(
		"select %s from `channels` where `channel_id`=?", sel,
	)
...
func ChannelExists(ctx context.Context, exec boil.ContextExecutor, channelID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `channels` where `channel_id`=? limit 1)"
