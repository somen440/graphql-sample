...
func testChannelsExists(t *testing.T) {
	...
	e, err := ChannelExists(ctx, tx, o.ChannelID)
	if err != nil {
...
func testChannelsFind(t *testing.T) {
	...
	channelFound, err := FindChannel(ctx, tx, o.ChannelID)
	if err != nil {
		t.Error(err)
	}
...
func testChannelToManyFollowedChannels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadFollowedChannels(ctx, tx, false, (*[]*Channel)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testChannelToManyAddOpFollowedChannels(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ChannelID != first.ChannelID {
			t.Error("foreign key was wrong value", a.ChannelID, first.ChannelID)
		}
		if a.ChannelID != second.ChannelID {
			t.Error("foreign key was wrong value", a.ChannelID, second.ChannelID)
		}
...
var (
	channelDBTypes = map[string]string{`ChannelID`: `varchar`, `Name`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_              = bytes.MinRead
)
